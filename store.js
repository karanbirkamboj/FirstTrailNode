...
jQuery(document).ready(function() {	
    ...
    $( document ).on( "click", ".payment_checked", function() {
        alert();
    });
    ...
    //	alert(id+'-'+category_id);
    	...
    	if ( single==2 || single !=2){    
    		if ( $("#website_disbaled_auto_cart").val()!="yes"){		    			
	    		...
	    		single_food_item_add(id, $(this).data("price"), $(this).data("size") , $(this).data("category_id"),  $(this).data("size_id") , $(this).data("discount") ,  $(this).attr('data-qty') );	
	    		return;
    		}
    	}
...
//alert();
    ...
    success: function(data){ 
    	...
    	if (data.code==1){
			...
			$(".cart_count").text(data.details.item_count);
		    $(".cart_count").html(data.details.item_count);
...
function addtocarts(vals)
{
    var vl='';
    if($(vals).data('icon') == 'minus')
    {
      vl =$(vals).data('qyt');
    ...
    $('.'+$(vals).data('class')).data('qty',vl);
    $('.'+$(vals).data('class')).attr('data-qty',vl);
    $('.'+$(vals).data('class')).click();
}

function single_food_item_add(item_id,price,size,category_id, size_id , discount ,qty)
{
	...
	{
	  params="merchant_id="+$(".merchant_ids").val();  
	} 
...
jQuery(document).ready(function($){
	...
	$( document ).on( "click", ".map-address", function() {
		...
		$( document ).on( "click", ".rate_site", function() {
		...
		$( document ).on( "click", ".search_subitems", function() {
		
			var text=$('.textaearchmenu').val();
			var sort=$('#default').val();
			var merchant=$('#merchant').val();
			var params="action=SearchmenuItems&currentController=store&text="+text+"&sort="+sort+"&merchant="+merchant;	
			params+= addValidationRequest();
			...
			{
			$('.menu-list-wrapper').html('No records found..!');
			}
			busy(false);
			}, 
			error: function(){	        	    	
		...
		$( document ).on( "click", ".searchbtnsub", function() {
		
			var text=$('.searchtextsub').val();
			var sort=$('#default').val();
			var params="action=SearchCategory&currentController=store&text="+text+"&sort="+sort;	
			params+= addValidationRequest();
			...
			if(data.code==1)
			{
			$('.category_list').html(data.details);
			}else
			{
			$('.category_list').html('No records found..!');
			}
			busy(false);
			}, 
			error: function(){	        	    	
...
$(document).ready(function(){
  $(".signup-click-show").click(function(){
    $(".login-main-box").slideUp(200);
    $(".sign-up-show").slideDown(200);
  });
  
  $(".login-click-show").click(function(){
    $(".login-main-box").slideDown(200);
    $(".sign-up-show").slideUp(200);
  });
  
  $(".forgot-click-show").click(function(){
    $(".logins-box").slideUp(200);
    $(".forgots-show").slideDown(200);
  }); 
  $(".back-logn").click(function(){
    $(".logins-box").slideDown(200);
    $(".forgots-show").slideUp(200);
  });
});
...
$(document).ready(function() {
	 $( document ).on( "click", ".minus", function() {    	
		...
		$('.itemid'+itemid).attr('data-qty',count);
		$input.change();
		return false;
	});
	  $( document ).on( "click", ".plus", function() {    	
		...
		var itemid=$(this).data('item');
		$('.itemid'+itemid).attr('data-qty',$input.val());
		$input.change();
		return false;
	});
});
...
$('.buttons').on('click', function(){
   var newurl = location.href +'/'+ $('.tack-order-by-id').val();
     newurl = newurl.replace('#','');
      newurl = newurl.replace('k//','k/');
   var id = $('.tack-order-by-id').val();
     id = id.replace('#','');
      if(id !='')
      {
     //   $(".order-main").load(newurl);
            $.get( newurl, function( data ) {
                ...
                {
                  $(".order-main").html( data );
                  $(".order-main").show();
                }
            });
      }else
      {
        $(".order-main").html( '<div class="row"><div class="col-md-6"><div class="heading-main"><h3>Order Status</h3></div></div></div>' );  
         $(".order-main").hide();
      }
});

$('.tack-order-by-id').on('keypress',function(e) {
    if(e.which == 13) {
       // $('.order-main').empty().load(location.href+ $(this).val());
     var newurl = location.href +'/'+ $(this).val();
     newurl = newurl.replace('#','');
      newurl = newurl.replace('k//','k/');
   var id = $(this).val();
     id = id.replace('#','');
...
$(function() {
            $('.accordion').find('.accordion-title').on('click', function(){
                ...
                $('.accordion-content').not(this).removeClass('show');
...
$('.textaearchmenu').on('keypress',function(e) {
    if(e.which == 13) {
        $('.search_subitems').click();
    }
});

$('.position').on('click',function(){
    if($('.close-btn.fas.fa-times').hasClass( "active" ))
    {
