...
if ( $("#sau_merchant_upload_file2").exists() ){						
	sau_merchant_progress2 = $("#sau_merchant_upload_file2").data("progress");
	sau_merchant_progress2 = $("."+sau_merchant_progress2);
	sau_merchant_preview2 = $("#sau_merchant_upload_file2").data("preview");
	
	sau_merchant_field2 = $("#sau_merchant_upload_file2").data("field");

	var uploader = new ss.SimpleUpload({
		 button: 'sau_merchant_upload_file2',
		 url: ajax_admin + "/uploadFile/?"+ addValidationRequest()+"&method=get&field="+sau_merchant_field2 + "&preview=" + sau_merchant_preview2  ,
		 name: 'uploadfile',			 	
		 responseType: 'json',			 
		 allowedExtensions: ['jpg', 'jpeg', 'png', 'gif'],			 
		 maxSize: image_limit_size,
		 ...
		 onSubmit: function(filename, extension) {		
		 	busy(true);
		 	this.setProgressBar(sau_merchant_progress2);
		 },
		 onComplete: function(filename, response) {			
		 	 busy(false); 	 
		 	 if (!response) {
                ...
                return false;            
             	...
             	dump(response);	             	
             	if( response.code==1){
             		$("."+sau_merchant_preview2).html( response.details.preview_html );
             	} else {
             		uk_msg( response.msg );
             	}
             }	            
...
jQuery(document).ready(function() {
    ...
    $( document ).on( "click", ".addnewprice2", function() {
    //	var html=$(".price_wrap2").html();  
    var html ='<div class="uk-grid"><div class="uk-width-1-3"><select class="uk-form-width-medium" name="size[]" id="size"><option value="0">large</option></select></div><div class="uk-width-1-3"><input class="uk-form-width-medium numeric_only" type="text" value="" name="price[]" id="price"></div><div class="uk-width-1-3"><a href="javascript:;" class="removeprice"><i class="fa fa-minus-square"></i></a></div></div>';
...
jQuery(document).ready(function() {
	...
	$( document ).on( "click", ".sau_remove_file2", function() {
	    preview2 = $(this).data("preview");			 
	    $("."+preview2).html('');
	});	
...
$("a.toggle-icns").click(function(){
  $("body").toggleClass("sidemenu-hide");
});

$(document).ready(function(){
 $('.dropdown-new').click(function(){
	  $(this).closest('.dropdown-new').toggleClass('short-menu');
...
if($(this).width()>768){
	$(selector).mCustomScrollbar(); //apply scrollbar with your options 
}else{
